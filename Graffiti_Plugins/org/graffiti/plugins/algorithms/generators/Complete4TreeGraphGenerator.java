 ...
 */
public class Complete4TreeGraphGenerator extends AbstractGenerator {
     ...
     */
    public Complete4TreeGraphGenerator(Graph graph, int numberOfNodes,
            boolean directed) {
        super();
        this.graph = graph;
    ...
    public String getName() {
        return "Graph Generator: Complete Tree of Degree 4";
    }
    ...
    public void execute() {
        ...
        if (numberOfNodes == 1) {
            ...
            int stop = (numberOfNodes / 4) - 1;
            if (numberOfNodes % 4 >= 2) {
                stop++;
            }

            for (int i = 0; i < stop; i++) {
                edges.add(addEdge(nodes.get(i), nodes.get((i * 4) + 1),
                        directed));
                edges.add(addEdge(nodes.get(i), nodes.get((i * 4) + 2),
                        directed));
                edges.add(addEdge(nodes.get(i), nodes.get((i * 4) + 3),
                        directed));
                edges.add(addEdge(nodes.get(i), nodes.get((i * 4) + 4),
                        directed));
            }
            ...
            if (numberOfNodes % 4 < 2) {
                incompleteNode = (numberOfNodes / 4) - 1;
            } else {
                incompleteNode = (numberOfNodes / 4);
            }

            edges.add(addEdge(nodes.get(incompleteNode), nodes
                    .get(numberOfNodes - 1), directed));

            switch (numberOfNodes % 4) {
            case 1:
                edges.add(addEdge(nodes.get(incompleteNode), nodes
