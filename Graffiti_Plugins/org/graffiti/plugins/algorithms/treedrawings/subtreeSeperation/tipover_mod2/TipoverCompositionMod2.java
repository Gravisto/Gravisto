...
//
// =============================================================================
// $Id: TipoverComposition.java 1548 2006-11-07 17:34:38Z keilhaue $

package org.graffiti.plugins.algorithms.treedrawings.subtreeSeperation.tipover_mod2;
...
/**
 * @author Andreas
 * @version $Revision: 1548 $ $Date: 2006-11-07 18:34:38 +0100 (Di, 07 Nov 2006)
 *          $
 */
public class TipoverCompositionMod2 extends TipoverComposition {
     ...
     *            children.
     */
    protected TipoverCompositionMod2(Node root, double rootWidth,
        ...
        if (childCompositions == null || childCompositions.size() == 0) {
            ...
            if (this.horizontal) {
                ...
                if (Util.isHelperNode(root)) {
                    topDistance = topDistance - nodeDistance / 2.0;
                }

                double maximumChildHeight = 0;
                for (int i = 0; i < this.subtrees.size(); i++) {

                    LayoutComposition currentComposition = this.subtrees.get(i);
                    double currentHeight = currentComposition.getHeight();

                    double currentRelYPos = topDistance;
                    if (Util.isHelperNode(currentComposition.getRoot())) {
                        currentRelYPos = topDistance - nodeDistance / 2.0;
                        currentHeight = currentHeight - nodeDistance / 2.0;
                        if (i != 0 || Util.isHelperNode(root)) {
                            currentRelXPos = currentRelXPos - nodeDistance
                                    / 2.0;
                        }
                    ...
                    this.subtreePositions.add(new Point2D.Double(
                            currentRelXPos, currentRelYPos));
                ...
                if (Util.isHelperNode(this.root)) {
                    this.relPositionOfRoot.setLocation(this.subtreePositions
                            .get(0).getX()
                            + this.subtrees.get(0).getWidth() / 2.0, 0.0);
                } else {
                    this.relPositionOfRoot.setLocation(this.width / 2.0,
                            this.rootHeight / 2.0);
                }
                ...
                double rightDistance = nodeDistance / 2.0 + this.rootWidth
                        / 2.0;

                if (Util.isHelperNode(this.root)) {
                    currentRelYpos = currentRelYpos - nodeDistance / 2.0;
                }
                ...
                for (LayoutComposition currentComposition : this.subtrees) {
                    ...
                    this.subtreePositions.add(new Point2D.Double(rightDistance,
                            currentRelYpos));
                ...
                if (nodeDistance / 2.0 + maximumChildWidth > this.rootWidth / 2.0) {
                    this.width = this.rootWidth / 2.0 + nodeDistance / 2.0
                            + maximumChildWidth;
                } else {
                ...
                this.height = currentRelYpos - nodeDistance;
            }
    ...
    public LayoutComposition instance(Node root,
        ...
        return new TipoverCompositionMod2(root, rootWidth, rootHeight,
    ...
    public List<LayoutComposition> instance(Node root,
        ...
        if (!nodesWithDimensions) {
            ...
            rootWidth = 0.0;// LayoutConstants.minNodeWidth;
            rootHeight = 0.0;// LayoutConstants.minNodeHeight;
        } else {
        ...
        resultList.add(new TipoverCompositionMod2(root, rootWidth, rootHeight,
    ...
    public void combineHelper(Point2D rootCompositionTotalPosition,
        ...
        if (onlyOneEdgeItr.hasNext()) {
            ...
            if (this.horizontal) {
                ...
                if (Util.isHelperNode(this.root)) {
                    totalY = totalY - this.nodeDistance / 2.0;
                }
    ...
    protected void setLayoutAttributesLocal(double orderNumber) {
        ...
        StringAttribute layoutTypeAttribute = new StringAttribute("type", "m2");
