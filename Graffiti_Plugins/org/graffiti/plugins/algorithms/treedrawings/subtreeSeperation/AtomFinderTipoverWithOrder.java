...
/**
 * This is a specialised algorithm for TipoverLayoutMod2 where width and height
 * of a LayoutComposition can depend on the order of the subtrees!
 ...
 */
public class AtomFinderTipoverWithOrder implements AtomFinder {
    /**
     * factory is used to instantiate LayoutComposition in this AtomFinder.
     */
    protected TipoverCompositionMod2 factory;
     ...
     */
    public AtomFinderTipoverWithOrder(TipoverCompositionMod2 factory,
    ...
    /**
     * This is a specialised algorithm for TipoverLayoutMod2 where width and
     * height of a LayoutComposition can depend on the order of the subtrees!
    ...
    public ArrayList<LayoutComposition> findAtoms(Node root) {
        if (root.getOutDegree() == 0) {
            ...
            // Get the orderNumber for each child node and insert them into
            // the TreeMap to order them by their order number...

            TreeMap<Double, Node> childNodesInCorrectOrder = new TreeMap<Double, Node>();

            // We introduce "negativeCounter" to deal with nodes that do not
            // have
            // an orderNumber.
            double negativeCounter = -1;
            for (Node currentChild : root.getAllOutNeighbors()) {

                double currentOrderNumber;
                try {
                    currentOrderNumber = currentChild
                            .getDouble("layout.orderNumber");
                } catch (AttributeNotFoundException a) {
                    currentOrderNumber = negativeCounter;
                    negativeCounter--;
                }

                childNodesInCorrectOrder.put(currentOrderNumber, currentChild);
            }

            ArrayList<ArrayList<LayoutComposition>> allChildrenAtoms = new ArrayList<ArrayList<LayoutComposition>>();
            for (Node currentChild : childNodesInCorrectOrder.values()) {
