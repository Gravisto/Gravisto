...
public class Triconnect extends AbstractAlgorithm implements
    ...
    private boolean isTriconnected;

    private Node root;

    private Node[] separationPair = new Node[2];
    ...
    private void startTriconnect() {
        isTriconnected = true;
        Biconnect biconnect = new Biconnect();
        biconnect.attach(graph);
        if (GUIMode) {
            biconnect.execute();
        } else {
            biconnect.testBiconnect();
        }
        if (!biconnect.isBiconnected()) {
            isTriconnected = false;
            if (GUIMode) {
                // System.out.println("\t Triconnected: " + isTriconnected);
            }
            return;
        }
        ...
        if (graph.getNumberOfNodes() > 1) {
            ...
            for (Iterator<Node> i = graph.getNodes().iterator(); i.hasNext()
                ...
                if (current != root) {
                    ...
                    root = graph.getNodes().get(1);
                    lowpoint(root);
                }
                isTriconnected = biconnect();
                if (!isTriconnected) {
                    done = true;
                    separationPair[1] = current;
                    if (GUIMode) {
    ...
    public AlgorithmResult getResult() {
        ...
        String text = "The graph is " + (isTriconnected() ? "" : "not ")
                + "triconnected.";
        result.setComponentForJDialog(text);
        result.addToResult("triconnected", isTriconnected());
        return result;
    }
