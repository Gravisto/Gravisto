...
public class NodeLabelLocator extends LabelLocator {
    ...
    private ArrayList<NodeLabelPosition> generatePositionCandidates(
            ...
            positionCandidates
                    .add(new NodeLabelPosition(
                            (NodeLabelPositionAttribute) nodeLabel
                                    .getPosition().copy(),
                            nodeGraphicAttributes, labelSize, this, 0d));
        }
        ...
        positionCandidates.add(new NodeLabelPosition(
                ...
                nodeGraphicAttributes, labelSize, this, 0.6d));
    ...
    @Override
    public void applyToLabel(TreeSet<LabelLocator> sortedLocators) {
        ...
        // {
        // candidatesSecondOrderCollisionCount =
        // candidate.getNumberOf2ndOrderCollisions();
        // candidatesCollisionCount = candidate.getNumberOfCollisions();
        // if (candidatesSecondOrderCollisionCount <
        // currentSecondOrderCollisionCount
        // || candidatesSecondOrderCollisionCount ==
        // currentSecondOrderCollisionCount
        ...
        // currentSecondOrderCollisionCount
        // && candidatesCollisionCount == currentCollisionCount
        // && candidate.getPreference() > currentQuality)
        // {
        // appliedCandidate = candidate;
        // currentQuality = appliedCandidate.getPreference();
...

        {
            ...
            for (NodeLabelPosition candidate : candidatePositions) {
                if (candidate.getQuality() > currentQuality) {
                    appliedCandidate = candidate;
        ...
        assert (this.placementQuality == appliedCandidate.getQuality());

        Statistics.locatorApplied(appliedCandidate.getQuality(),
                appliedCandidate.getNumberOf2ndOrderCollisions(),
                appliedCandidate.getNodeOverlaps(), appliedCandidate
                        .getEdgeOverlaps(), appliedCandidate
                        .getNumberOfCollisions(), appliedCandidate
                        .getPreference());
        ...
        // Mark still residing conflicts as second order conflicts
        {
            ...
            for (Iterator<LabelPosition> conflictPartnerIt = appliedCandidate
                ...
                candidate = conflictPartnerIt.next();
                // avoid concurrent modifications
                boolean isRemaining = sortedLocators
                        .remove(candidate.parentLocator);
                candidate.mark2ndOrderCollision();
                if (isRemaining) {
                    sortedLocators.add(candidate.parentLocator);
                }
            }
        }
        ...
        for (LabelPosition candidate : candidatePositions) {
            ...
            candidate.unmarkAllCollisions(sortedLocators);
            // }
        }
    ...
    // {
    // int numberOfCollisions = Integer.MAX_VALUE;
    // for (LabelPosition candidate : candidatePositions) {
    // if (candidate.getNumberOfCollisions() < numberOfCollisions) {
    // numberOfCollisions = candidate.getNumberOfCollisions();
    // }
    // }
