...
public class TernaryTreesInHexa2LeafsOut_radial extends TreeInHexa2 {
    ...
    private void drawTree(Node placed, int level, int side) {
        ...
        // Array für die Söhne des aktuellen Knotens <code>placed</code>
        Node[] sons = new Node[0];

        sons = placed.getOutNeighbors().toArray(sons);

        // drei Söhne des aktuellen Knotens
        if (sons.length == 3) {
            if (side == 2) {
                this
        ...
        // zwei Söhne des aktuellen Knotens
